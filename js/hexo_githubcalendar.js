...
var responsiveChart = () => {
        ...
        var weekCount = git_data.length;
        var height = 9 * 0.96 * canvasWidth / weekCount;
        
        // 2. 设置Retina高清适配
        // 物理像素 = 逻辑像素 * devicePixelRatio
        github_calendar_c.width = canvasWidth * ratio;
        github_calendar_c.height = height * ratio;
        // CSS样式保持逻辑像素大小
        github_calendar_c.style.width = canvasWidth + 'px';
        github_calendar_c.style.height = height + 'px';
        // 移除 ctx.scale，改为手动计算坐标，避免状态丢失问题
        // github_calendar_ctx.scale(ratio, ratio);

        // 3. 计算绘图参数（全部乘以 ratio 转换为物理像素）
        var weekWidth = (canvasWidth * 0.96 / weekCount) * ratio;
        var linemaxwitdh = (height / 9) * ratio;
        var lineminwitdh = 0.8 * linemaxwitdh;
        var setposition = {x: 0.02 * canvasWidth * ratio, y: 0.025 * canvasWidth * ratio};
        ...
        for (var week = 0; week < git_data.length; week++) {
          ...
          for (var day in weekdata) {
            ...
            dataitem.x = setposition.x; // 这里记录的是物理坐标，用于Tooltip可能需要转换，但Tooltip是用clientX/Y的，不受影响
            dataitem.y = setposition.y;
            github_calendar_ctx.fillRect(setposition.x, setposition.y, lineminwitdh, lineminwitdh);
            setposition.y = setposition.y + linemaxwitdh
          }
          setposition.y = 0.025 * canvasWidth * ratio;
          setposition.x = setposition.x + weekWidth;
        }
        if (document.body.clientWidth > 700) {
          // 字体大小也需要乘 ratio
          github_calendar_ctx.font = "600 " + (10 * ratio) + "px Arial";
        ...
        var getMousePos = (canvas, event) => {
          ...
          for (var item of git_positionplusdata) {
            ...
            fetch(git_githubapiurl).then(data => data.json()).then(data => {
                ...
                git_monthindex = git_thisday.substring(5, 7) * 1; // 当前月份 (1-12)
